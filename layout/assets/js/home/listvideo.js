function listVideo() {
  ...
  recent_video_addBtn.addEventListener('click', () => {
    recent_video_box.insertBefore(createLoadingBox(), recent_video_addBtn)
    for(let i=0;i<8;i++) {
      if(recent_video_data[i].nextPage) {
        let next_body = [recent_video_data[i].user_id, recent_video_data[i].nextPage]
  ...
  function add_MyRecentVideo(data, target, last) {
    ...
    recent_video_manageBtn.forEach((elem, idx) => {
      elem.addEventListener('click', () => {
        let response_text = manageVideoToggle(recent_video_iconInfo[idx].querySelector('.video_id').innerHTML)
